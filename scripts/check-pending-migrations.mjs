...
const MARKERS = [
  ...
  { id: "20260516000000", label: "Inquiries & complaints", sql: "SELECT 1 FROM information_schema.tables WHERE table_schema='public' AND table_name='inquiries' LIMIT 1" },
  ...
  { id: "20260604030000", label: "Student panel fixes", sql: "SELECT proname FROM pg_proc WHERE proname = 'rpc_student_home' LIMIT 1" },
  ...
  { id: "20260604100000", label: "Battleground phase 4", sql: "SELECT proname FROM pg_proc WHERE proname = 'rpc_battleground_feed' LIMIT 1" },
...
async function rpcExists(url, key, fn) {
  ...
  const body = await r.text();
  if (body.includes("Could not find the function") || body.includes("function") && body.includes("does not exist"))
    return false;
  return true;
}
...
async function probeViaRest() {
  ...
  for (const m of MARKERS) {
    ...
    if (m.id === "20260604030000") {
      const ok = await rpcExists(url, key, "rpc_student_home");
      (ok ? applied : pending).push({ id: m.id, label: ok ? label : label + " (or marker RPC missing)" });
      continue;
    }
    ...
    if (m.id === "20260604100000") {
      const ok = await rpcExists(url, key, "rpc_battleground_feed");
      (ok ? applied : pending).push({ id: m.id, label });
      continue;
    }
