...
const ALLOWLIST = {
  ...
  rpc_student_recovery_queue: "No parameters; self-scoped via auth.uid(): refuses a null caller and returns _recovery_queue_for(auth.uid()) — the caller's own open student_mistakes, chapter_state and recovery rounds (20261101000000).",
  ...
  _still_skipped_questions: "Internal (REVOKEd from anon/authenticated). Reads question_attempts WHERE user_id = its _uid argument only; its two callers (rpc_my_skipped_questions, rpc_my_skipped_by_chapter) pass auth.uid() and nothing else.",
  ...
  rpc_my_skipped_by_chapter: "Scoped to auth.uid(): passes it to _still_skipped_questions; question_bank and topics are G2 GLOBAL catalogue tables (no school_id). Takes no arguments.",
  rpc_student_chapter_analysis: "DROPPED by 20261087000000_analysis_counts_the_questions_you_still_skipped.sql; nothing called it. The entry stays because this lint reads migration FILES and 20261057000000/20261058000000 still contain the CREATE -- migration history is append-only. While it existed the reason was: every per-student table read WHERE user_id = auth.uid(); chapters, topics and question_bank are global catalogue tables; no arguments.",
  ...
  _student_difficulty_rank:
    "Owner-scoped by construction, and tighter than a school predicate. It takes ONE argument, a chapter id, and reads `auth.uid()` itself — there is no user-id parameter to point at another student. Both of its queries filter `qa.user_id = _uid` over question_attempts joined to question_bank (G2 GLOBAL, no school_id column), and it returns a single integer 1-3: the mean difficulty of the questions this caller has answered, which is a fact about the caller and nobody else. A same_school() predicate would widen nothing and would restate a fence that auth.uid() already closes (G9), and §10.8 makes practice the student's own rather than their school's — the same reason rpc_student_practice_analytics carries above. With no JWT subject it returns 2 (medium) without reading a row. Checkable: the body must contain `_uid uuid := auth.uid()`, every FROM public.question_attempts must be followed by `qa.user_id = _uid`, and it must declare no _user_id/_student_id parameter. Applied 2026-09-25 (20261100000000); re-read live the same day.",
  send_learning_reminders:
    "Platform maintenance job, not a request handler: the 'learning-reminders' cron entry (daily, 04:00 UTC = 09:30 IST, 20261102000000) writes ONE batched revision/recovery reminder per student per day (§4.1b, §5, §9). It takes no parameters at all, so there is nothing a caller could point at another tenant, and there is no single institution to scope a platform-wide sweep to — it must consider every student. Nothing is mixed: each notification it writes names its own recipient, and _notify derives that row's school_id from the recipient's own profile, so the tenant of every row is the student's own. It reads chapter_state, chapter_tally, recovery_sessions, student_mistakes and notifications, each filtered to the one student it is writing for (user_id = _u), chapters (G2 GLOBAL) for the name, and that student's own Recovery card through _recovery_queue_for(_u); it writes nothing but notifications. Cadence rewritten by 20261106000000. Reachable only by cron: EXECUTE is revoked from PUBLIC, anon and authenticated in the same migration. Same shape and same reason as dispatch_notification_push beside it. Applied 2026-09-25; its proof rolls back everything it writes.",
  _recovery_queue_for:
    "Internal, not a request handler: EXECUTE is revoked from PUBLIC, anon and authenticated (20261101000000), so no client can name a student to it. It is the body rpc_student_recovery_queue always had, with auth.uid() lifted into a parameter; its two callers pass a student's own id — rpc_student_recovery_queue passes auth.uid() (after refusing a null one), and the learning-reminders cron passes each student in turn to write that student's own reminder. Every per-student table it reads is filtered to that one id: student_mistakes, chapter_state and recovery_sessions by user_id = _uid, and _recovery_session_plan_for(_uid, chapter) the same way; chapters and curriculum_subjects are G2 GLOBAL catalogue tables. Owner-scoped, which is tighter than a school predicate. Checkable: pg_proc.proacl must not grant EXECUTE to authenticated or anon, and every FROM of a per-student table in the body must carry `user_id = _uid`.",
  ...
  _recovery_step_pool:
    "Internal, not a request handler: EXECUTE is revoked from PUBLIC, anon and authenticated (20261104000000); its one caller is _recovery_session_plan_for, which passes the student it is planning for and the source it built from that student's own open mistakes. It reads question_bank (G2 GLOBAL, no school_id column), and reads question_attempts and student_mistakes ONLY as `qa.user_id = _uid` / `sm.user_id = _uid` — to rank what that student has not seen and to exclude what they got wrong — returning question ids from the shared bank, never another student's rows. Owner-scoped, which is tighter than a school predicate. Checkable: pg_proc.proacl must not grant EXECUTE to authenticated or anon; every FROM of question_attempts or student_mistakes in the body must carry `user_id = _uid`.",
  _enqueue_variant_generation:
    "SECURITY DEFINER, and callable by nobody: 20261012000000 REVOKEs it from anon and authenticated, so the only caller is _ensure_recovery_session inside the practice-finish path. It reads question_bank and student_upload_questions (only the one upload a plan source names, to see whether it was answered from its file) and writes variant_generation_queue. question_bank is a G2 GLOBAL table (7A: 'Shared across every school. No institution_id'); the queue is keyed on (source question or source upload, tier) and deliberately not on the student, because §4.2a's economics depend on one generation serving every student who fails that question — adding a school_id would be the bug, not the fix. The jsonb plan it is handed is produced by _recovery_session_plan_for for one named student, so every question id and upload id it can reach is that student's own mistake, or the bank question one of them was matched to (20261105000000).",
  rpc_enqueue_upload_variant_generation:
    "SECURITY DEFINER owner door for §10 upload promotion (20261068000000). Authenticated may EXECUTE; the body requires auth.uid() and SELECTs student_upload_questions WHERE id = _upload_question_id AND owner_id = auth.uid(), so another account's private question cannot be enqueued. It writes variant_generation_queue with source_upload_question_id set and source_question_id NULL — the queue has no school_id by design (same as bank-sourced jobs: a variant is keyed on the source question, not the student). Reads question_bank only to skip when a promoted variant already exists for that upload source+tier. No cross-tenant leak path: upload rows are owner-fenced before the queue write.",

  // --- Upload / exam match doors, 2026-09-24 ---
  match_question_bank_for_exam:
    "Reads question_bank (and topics) only — a G2 GLOBAL table with NO school_id column at all (7A: 'Shared across every school. No institution_id'), so there is no tenant predicate available to add. Same table and same reason as _recovery_variant_pool / rpc_practice_bank_catalog. LANGUAGE sql STABLE with no SECURITY DEFINER clause, so it is INVOKER; the board/approval filters that matter live in qb_select_approved_board and apply because this runs as the caller. Scope is by p_exam_id (CUET individuals have class_id NULL), deliberately NOT a loosened escape on match_question_bank which keeps its p_school_id (G14). No p_exam_id IS NULL escape: unknown exam returns nothing. EXECUTE revoked from PUBLIC/anon, granted to authenticated and service_role (20261064000000 verify). Checkable: body must contain `qb.exam_id = p_exam_id` and `p_exam_id IS NOT NULL`, and must NOT contain a school_id parameter.",
  rpc_dispute_ai_upload_answer:
    "SECURITY DEFINER owner door for §6.1 AI-upload answer dispute (20261065000000). Authenticated may EXECUTE; anon revoked. Body requires `_uid := auth.uid()`, SELECTs student_upload_questions WHERE id = _upload_question_id AND owner_id = _uid (non-owner gets 'not found'), then UPDATEs only student_mistakes and question_attempts WHERE user_id = _uid — OWNER-scoped, strictly tighter than same_school(). Takes one argument (the upload question id); no target-user parameter. Refuses non-AI answer_source. Migration verify asserts owner_id = _uid, answer_source, excluded_from_accuracy, and cleared status. Adding school_id would restate a fence the owner predicates already impose (G9).",
  dispatch_variant_generation:
    "SECURITY DEFINER cron drain, REVOKEd from anon and authenticated by 20261012000000 (body refreshed 20261068000000 to resolve upload-sourced jobs); its only caller is the pg_cron job drain-variant-generation, running as postgres. It touches variant_generation_queue (no school_id — see above) and reads question_bank (G2 global, no school_id column exists) to resolve finished jobs. It reads no per-student table at all: a job names a QUESTION and a tier, never a user, which is the whole point of caching by question. The one credential it handles is the vault secret variant_generation_drain, deliberately not the service-role key, so a reader of pg_proc finds a single-purpose token rather than a master one. Same shape as dispatch_notification_push, already in production.",
  rpc_revision_session_plan:
    "SECURITY DEFINER since 20261011000000 (the student may actually call it); body refreshed by 20261100000000. Owner-scoped by construction: `_uid uuid := auth.uid()` with a RAISE when it is null, and every per-student read carries it — student_mistakes WHERE sm.user_id = _uid, the unseen half's NOT EXISTS over question_attempts (qa.user_id = _uid) and student_mistakes (sm.user_id = _uid), chapter_state WHERE user_id = _uid; question_bank is G2 GLOBAL with no school_id column. It takes one argument, a chapter id, never a user id, so there is nothing to point at another student. Entitlement is enforced BEFORE any read by _recovery_chapter_is_mine, which raises rather than filtering. The level it draws around comes from _student_difficulty_rank, which reads auth.uid() itself. Re-read live 2026-09-25.",
