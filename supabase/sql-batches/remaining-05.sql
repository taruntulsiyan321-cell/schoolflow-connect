-- REMAINING BATCH 5/5 — Supabase SQL Editor
...
BEGIN
  UPDATE public.dpp_questions dq SET
    class_level = COALESCE(dq.class_level, c.class_level, d.class_level),
...
BEGIN
  UPDATE public.battle_questions bq SET
    concept = COALESCE(NULLIF(bq.concept, ''), NULLIF(qb.concept, ''), NULLIF(qb.topic, ''), NULLIF(b.chapter, ''), b.subject),
    subconcept = COALESCE(NULLIF(bq.subconcept, ''), NULLIF(qb.subconcept, ''), NULLIF(qb.topic, ''), bq.concept)
  FROM public.battles b
  LEFT JOIN public.question_bank qb ON qb.id = bq.bank_question_id
  WHERE bq.battle_id = b.id AND (bq.concept IS NULL OR bq.concept = '');
  GET DIAGNOSTICS _n = ROW_COUNT;
  RETURN _n;
END; $$;
