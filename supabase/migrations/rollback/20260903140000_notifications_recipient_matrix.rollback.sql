-- ═══════════════════════════════════════════════════════════════════════════
-- ROLLBACK — process_academic_event returns to its pre-convergence body
--
-- Restores, exactly:
--   · marks.published / marks.updated as the bare `NULL;` no-op
--   · the six off-spec _notify_school_operators fan-outs
--   · the per-absence "A student was marked …" principal notification
--   · test.scheduled / test.published, examination.deleted, doubt.solved,
--     battle.finished, and the empty battle.created / battle.joined branches
-- and removes the leave.requested and leave.reviewed branches.
--
-- WHAT THIS COSTS, PLAINLY: leave decisions stop notifying anyone, which §10.12
-- lists among the notifications a student cannot turn off; and marks.published
-- goes silent again for every student and parent, which both §10.12 and §10.15
-- require. Running this reopens two spec violations. It is here because a
-- migration must be reversible, not because reversing it is safe.
--
-- Notification rows already written are NOT removed. They are correct rows
-- about real events, and deleting a student's "Marks published" because the
-- handler was rolled back would be a worse lie than never sending it.
-- ═══════════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  BEGIN
    ...
    ELSIF e.event_type LIKE 'homework%' AND e.class_id IS NOT NULL AND e.student_id IS NULL
      ...
      LOOP
        INSERT INTO public.academic_events (
          ...
          e.school_id, 'student.profile.refresh_requested', 'student_academic_profile',
          r.id, e.actor_user_id, r.id, e.class_id,
          jsonb_build_object('source_event', e.id, 'source_type', e.event_type), 'pending'
        );
      ...
      IF _enqueued >= 100 AND _last_student IS NOT NULL THEN
        INSERT INTO public.academic_events (
          ...
          e.school_id, 'homework.class.refresh_chunk', 'homework', e.entity_id,
          e.actor_user_id, NULL, e.class_id,
          jsonb_build_object(
    ...
    ELSIF e.event_type IN (
            'examination.finalized', 'marks.results_published',
            'examination.scheduled', 'examination.deleted'
          )
    ...
    ELSIF e.event_type IN ('homework.assigned', 'homework.published') AND e.class_id IS NOT NULL THEN
      ...
      PERFORM public._notify_school_operators(e.school_id, 'homework', _title, _body, 'book-open', '/principal');
    ...
    ELSIF e.event_type IN ('test.scheduled', 'test.published') AND e.class_id IS NOT NULL THEN
      ...
      PERFORM public._notify_school_operators(e.school_id, 'test', _title, _body, 'clipboard-list', '/principal');
    ELSIF e.event_type = 'test.attempt.completed' AND e.student_id IS NOT NULL THEN
      ...
      END;
      PERFORM public._notify_student_parents(e.student_id, 'test', _title, _body, 'clipboard-check', '/parent');
    ELSIF e.event_type = 'examination.scheduled' AND e.class_id IS NOT NULL THEN
      ...
      PERFORM public._notify_school_operators(e.school_id, 'exam', _title, _body, 'calendar', '/principal');
    ...
    ELSIF e.event_type = 'marks.results_published' AND e.class_id IS NOT NULL THEN
      ...
      PERFORM public._notify_school_operators(e.school_id, 'result', _title, _body, 'clipboard-check', '/principal');
    ...
    ELSIF e.event_type IN ('doubt.created', 'doubt.replied', 'doubt.solved') THEN
      ...
      IF e.event_type = 'doubt.solved' AND e.student_id IS NOT NULL THEN
        PERFORM public._notify_student_parents(
          e.student_id, 'doubt', 'Doubt solved',
    ...
    ELSIF e.event_type = 'battle.finished' THEN
      IF e.student_id IS NOT NULL THEN
        ...
        _body := coalesce(e.payload->>'title', 'Score ' || coalesce(e.payload->>'score', '0'));
        PERFORM public._notify_student_circle(e.student_id, 'battle', _title, _body, 'swords', '/student/battleground');
      ELSIF e.actor_user_id IS NOT NULL THEN
        _title := 'Battle finished';
        _body := coalesce(e.payload->>'title', 'Your battle results are ready');
        PERFORM public._notify(e.actor_user_id, 'battle', _title, _body, 'swords', '/student/battleground');
      END IF;
    ...
    ELSIF e.event_type = 'badge.earned' THEN
      ...
      _xp_user := coalesce(
        (e.payload->>'user_id')::uuid, e.actor_user_id,
        (SELECT user_id FROM public.students WHERE id = e.student_id LIMIT 1)
      );
      IF _xp_user IS NOT NULL THEN
        PERFORM public._notify(_xp_user, 'badge', _title, _body, 'award', '/student/achievements');
      END IF;
      IF e.student_id IS NOT NULL THEN
        PERFORM public._notify_student_circle(e.student_id, 'badge', _title, _body, 'award', '/parent');
      END IF;
    ...
    IF e.event_type NOT IN ('student.profile.refresh_requested', 'homework.class.refresh_chunk')
      ...
      INSERT INTO public.school_activity_feed (
        ...
        e.school_id, e.actor_user_id, e.event_type, e.entity_type, e.entity_id,
...
DELETE FROM public.schema_migrations
 WHERE version = '20260903140000_notifications_recipient_matrix';
