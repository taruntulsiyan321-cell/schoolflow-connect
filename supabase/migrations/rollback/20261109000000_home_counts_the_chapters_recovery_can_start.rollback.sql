-- ===========================================================================
-- ROLLBACK: recovery_pending counts `ready` again
--
...
--
-- Undoes: 20261109000000_home_counts_the_chapters_recovery_can_start.rollback.sql
-- ===========================================================================
...
BEGIN
  ...
  _new := replace(_def,
   ...
   WHERE (q->>'startable')::boolean;$old$,
    $new$  SELECT count(*)::int INTO _recovery_pending
    FROM jsonb_array_elements(public.rpc_student_recovery_queue()) q
   WHERE (q->>'ready')::boolean;$new$);
  IF _new = _def THEN
    RAISE EXCEPTION 'could not find the startable recovery_pending count in rpc_student_academic_snapshot';
  END IF;
  EXECUTE _new;
END $snap$;
