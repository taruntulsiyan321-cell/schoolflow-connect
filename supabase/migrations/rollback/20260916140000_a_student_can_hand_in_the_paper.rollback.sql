-- Rollback for 20260916140000.
--
-- Removes the DEFAULT from _answers, WHICH BREAKS TEST SUBMISSION AGAIN: the
-- attempt screen calls rpc_test_submit with only _attempt_id, and PostgREST
-- cannot resolve a one-argument call against a two-required-argument function.
-- Dropping first because CREATE OR REPLACE cannot remove a default.
