-- A VARIANT IS NOT ITS OWN MISTAKE.
--
-- THE REPORT: "A wrong answer to a new variant question isn't counted. It
-- should add to the original mistake's times wrong count."
--
-- MEASURED 2026-09-22 against production:
--
--   147 variants in the bank (question_bank.source_question_id set)
--   226 attempts on them, 49 of those wrong
--    20 of the 49 were in recovery mode
--    10 student_mistakes rows are keyed on a VARIANT rather than its origin
--
-- TWO DEFECTS, ONE CAUSE — the attempt was never resolved to the question
-- the variant was generated from.
--
--   IN RECOVERY: §4.6 says recovery may bump a mistake but never add one.
--   The guard implemented that by looking for a mistake keyed on the
--   attempted question; for a variant there is none, so it recorded NOTHING.
--   "Never add one" became "never do anything", and the bump was lost with
--   the insert. Those 20 wrong answers left no trace.
--
--   OUTSIDE RECOVERY: the ordinary path keyed the mistake on the variant
--   itself, which is where the 10 rows came from. One weakness fragments
--   across a parent and its generated children and neither row carries the
--   true times_wrong.
--
-- THE FIX is one resolution, before the guard rather than inside it:
-- _mistake_qid is COALESCE(question_bank.source_question_id, _bank_id), so a
-- variant resolves to its origin and every other question resolves to
-- itself. The guard then finds the ORIGINAL's row and bumps it, which is
-- what §4.6 asked for all along; and ordinary practice stops minting
-- variant-keyed rows.
--
-- rpc_record_concept_mistake is already an upsert on
-- (user_id, source, question_id) — insert with times_wrong 1, else bump — so
-- passing it the origin is all that was needed.
--
-- THE 10 EXISTING VARIANT-KEYED ROWS ARE NOT TOUCHED HERE. Folding them into
-- their parents means merging times_wrong, last_wrong_at and status across
-- rows that may both be open, and deciding what happens when only the child
-- is resolved. That is a data repair with its own rollback, not a side
-- effect of a function change.
...
BEGIN
  IF _src LIKE '%rpc_record_concept_mistake(%' AND _src NOT LIKE '%_session_id, _mistake_qid,%' THEN
    RAISE EXCEPTION 'the mistake is still keyed on the attempted question, not its origin';
  END IF;
  IF _src NOT LIKE '%COALESCE(qb.source_question_id, _bank_id)%' THEN
    RAISE EXCEPTION 'a variant is no longer resolved to the question it came from';
  END IF;
END
$guard$;

INSERT INTO public.schema_migrations (version)
VALUES ('20261051000000_a_variant_is_not_its_own_mistake')
