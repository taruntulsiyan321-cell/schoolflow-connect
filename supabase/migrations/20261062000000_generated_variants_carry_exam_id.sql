-- ===========================================================================
-- Generated variants inherit exam_id from their source question.
-- Without this, CUET (exam) source mistakes produce school-bank variants
-- (exam_id NULL) that individual students cannot read via qb_select_approved_board.
-- ===========================================================================
...
COMMENT ON FUNCTION public.store_generated_questions(jsonb) IS
  'The one way an AI-generated question enters question_bank. Topic (or source) required; chapter/subject/class/board derived from curriculum; exam_id inherited from source when present. service_role only.';

-- Backfill: any existing AI variant whose source is exam-tagged but the row is not.
UPDATE public.question_bank v
   SET exam_id = s.exam_id,
       board = COALESCE(v.board, s.board)
  FROM public.question_bank s
 WHERE v.source_question_id = s.id
   AND v.source_type = 'ai_generated'
   AND s.exam_id IS NOT NULL
   AND v.exam_id IS DISTINCT FROM s.exam_id;
...
DECLARE
  ...
  _new uuid;
BEGIN
  SELECT id INTO _exam FROM public.competitive_exams WHERE code = 'cuet' AND is_active LIMIT 1;
  IF _exam IS NULL THEN
    RAISE NOTICE 'proof skipped: no cuet exam';
    RETURN;
  END IF;

  SELECT id INTO _src
    FROM public.question_bank
   WHERE exam_id = _exam AND topic_id IS NOT NULL AND is_active
   LIMIT 1;
  IF _src IS NULL THEN
    RAISE NOTICE 'proof skipped: no CUET source with topic';
    RETURN;
  END IF;

  -- Dry-run inheritance via a rolled-back insert is heavy; assert column is on the function body instead.
  IF position('exam_id' IN pg_get_functiondef('public.store_generated_questions(jsonb)'::regprocedure)) = 0 THEN
    RAISE EXCEPTION 'proof: store_generated_questions body has no exam_id';
  END IF;
END
$proof$;
