-- ===========================================================================
-- THE RECOVERY REMINDER COUNTS ITS SESSIONS
--
-- The first reminder sent (2026-09-25, the CUET audit account) read "10
-- chapters are ready to review — Your recovery session is built and waiting."
-- Ten chapters, ten sessions, one "session". The body now agrees with the
-- title's count. Nothing else in send_learning_reminders changes.
--
-- ROLLBACK: rollback/20261103000000_the_recovery_reminder_counts_its_sessions.rollback.sql
-- ===========================================================================
...
DO $proof$
DECLARE _src text := pg_get_functiondef('public.send_learning_reminders()'::regprocedure);
BEGIN
  IF _src NOT LIKE '%built and waiting for each of them%' THEN
    RAISE EXCEPTION 'the reminder does not count its sessions';
  END IF;
END
$proof$;
