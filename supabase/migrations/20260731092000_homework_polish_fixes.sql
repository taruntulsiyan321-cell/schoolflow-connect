-- ============================================================================
-- Homework polish — profile math, unpublish sync, fan-out remainder,
-- resubmit semantics, teacher notify fallback, parent_students RLS, status default
-- ============================================================================

-- ── 0. Status default must match CHECK (was 'active') ────────────────────────
ALTER TABLE public.homework ALTER COLUMN status SET DEFAULT 'draft';

-- ── 1. Profile: exclude returned from completion; scope to published HW ──────
...
BEGIN
  ...
  -- Completion excludes returned (still actionable) and only counts published HW
  ...
  -- Pending = assigned − complete − returned (returned still open)
...
COMMENT ON FUNCTION public.refresh_student_academic_profile(uuid) IS
  'Homework polish: completion excludes returned; metrics scoped to published homework';

-- ── 2. Emit homework.unpublished on published→draft ─────────────────────────
...
-- ── 3. Resubmit only after a prior turn-in (not pending→submitted) ───────────
...
-- ── 4. Sync: unpublish fan-out, >100 continuation, teacher notify fallback ───
...
BEGIN
  ...
  BEGIN
    ...
    ELSIF e.event_type LIKE 'homework%' AND e.class_id IS NOT NULL AND e.student_id IS NULL
      ...
      -- Continue remainder via chunk event (process_pending drains these)
    ...
    ELSIF e.event_type IN ('homework.submitted', 'homework.resubmitted', 'homework.submission.created') THEN
      ...
      ELSIF _hw_class IS NOT NULL THEN
        -- Fallback: notify all teachers mapped to the class
        FOR r IN
...
-- ── 5. Parent RLS: also allow parents↔parent_students links ────────────────
DROP POLICY IF EXISTS "Parents via parent_students can view homework" ON public.homework;
CREATE POLICY "Parents via parent_students can view homework"
...
CREATE POLICY "Parents via parent_students can view submissions"
  ...
  USING (
    EXISTS (
      ...
      JOIN public.parent_students ps ON ps.parent_id = p.id
      WHERE p.user_id = auth.uid()
        AND ps.student_id = homework_submissions.student_id
    )
  );

-- ── 6. Guard: students cannot write grade / teacher review columns ───────────
...
CREATE TRIGGER trg_homework_submission_student_guard
  BEFORE UPDATE ON public.homework_submissions
  FOR EACH ROW EXECUTE FUNCTION public.tg_homework_submission_student_guard();
