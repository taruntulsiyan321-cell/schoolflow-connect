...
function RevItemCard({
  ...
  return (
    <GlassCard className="p-4 hover:border-black/15 transition-all">
      ...
      <div className="flex items-center gap-2 mt-3 flex-wrap">
        ...
        <Link to="/student/aicoach"
          className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-black/5 border border-black/10 text-violet-300 text-xs font-semibold hover:bg-black/10 transition-all">
    ...
    <div className="space-y-5">
      <GlassCard className="p-8 text-center">
        ...
        <div className="flex flex-wrap justify-center gap-2">
          ...
          <Link
            to="/student/aicoach"
            className="px-4 py-2 rounded-xl bg-black/5 border border-black/10 text-violet-300 text-sm font-semibold"
          >
            Ask Nova
          </Link>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid sm:grid-cols-3 gap-3">
        <div
          className="p-4 rounded-2xl border border-black/7 bg-muted text-left opacity-60"
          ...
          {(["all","due","upcoming"] as const).map(f => (
            <button key={f} onClick={() => setFilter(f)}
              className={cn("px-3 py-1.5 rounded-xl text-xs font-semibold capitalize transition-all",
                filter === f ? "bg-violet-500/20 border border-violet-500/40 text-violet-300" : "bg-black/5 border border-black/10 text-muted-foreground hover:bg-black/10")}>
              {f === "due" ? "Due Today" : f}
            </button>
          ))}
        </div>
          ...
          {subjects.map(s => (
            <button key={s} onClick={() => setSubjectTab(s)}
              className={cn("px-2.5 py-1 rounded-lg text-xs font-semibold transition-all",
                subjectTab === s ? "bg-black/15 border border-black/25 text-foreground" : "bg-black/5 border border-black/8 text-muted-foreground hover:bg-black/10")}>
              {s}
            </button>
          ))}
        </div>
