        ...
        {filtered.map((t) => (
          <button key={t.id} onClick={() => onSelect(t.id)}
            className={cn("w-full flex items-start gap-3 px-4 py-3 hover:bg-white/3 transition-all text-left",
              selectedId === t.id && "bg-[#f59e0b]/5 border-r-2 border-[#f59e0b]")}>
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2">
                ...
                {t.unreadCount > 0 && <div className="w-4 h-4 rounded-full bg-[#f59e0b] text-black text-[8px] font-black flex items-center justify-center shrink-0">{t.unreadCount}</div>}
              </div>
          ...
          return (
            <div key={m.id} className={cn("flex gap-3", isMe && "flex-row-reverse")}>
              ...
              <div className={cn("max-w-[70%]", isMe && "text-right")}>
                <div className={cn("text-[8px] font-semibold mb-1", isMe ? "text-[#f59e0b]" : "text-[#78788c]")}>{m.from}</div>
                <div className={cn("px-3 py-2 rounded-2xl text-xs leading-relaxed",
                  isMe ? "bg-[#f59e0b]/15 text-[#fcd34d]" : "bg-white/5 text-[#b0b0c0]")}>
                  {m.body}
                </div>
        ...
        <div className="flex items-end gap-2">
          ...
          <div className="flex flex-col gap-2">
            <button onClick={send} disabled={!input.trim()}
              className="w-9 h-9 rounded-xl bg-[#f59e0b] text-black flex items-center justify-center hover:bg-[#d97706] disabled:opacity-40 transition-all">
      ...
      <div className="w-72 shrink-0 bg-[#131316] border-r border-white/7 flex flex-col">
        <div className="flex items-center justify-between px-4 py-3 border-b border-white/7">
          ...
          <button onClick={() => setShowNew(true)}
            className="w-7 h-7 rounded-lg bg-[#f59e0b]/15 text-[#f59e0b] flex items-center justify-center hover:bg-[#f59e0b]/25 transition-all text-base font-bold">
            +
          </button>
        </div>
        ...
        <div className="px-4 py-3 border-b border-white/7">
          ...
          {assignedClasses.map((c) => (
            <button key={c.id}
              ...
              <Users className="w-3.5 h-3.5 text-[#46465a] group-hover:text-[#f59e0b] transition-all" />
