...
export function ExplainPanel(props: Props) {
  ...
  const load = async () => {
    ...
    try {
      ...
      // 2) Ask the AI
      const { data: res, error: fnErr } = await supabase.functions.invoke("ai-explain", {
        body: {
      ...
      const payload: Explanation = {
        ...
        why_wrong: (res as any)?.why_wrong ?? "",
        concept: (res as any)?.concept ?? "",
        how_to_improve: (res as any)?.how_to_improve ?? "",
      };
      setData(payload);
        ...
        <div className="rounded-xl border border-border/70 bg-muted/30 p-4 animate-rise">
          ...
          {error && (
            <div className="text-sm text-destructive flex items-center gap-2">
              ...
              <button onClick={() => { setError(null); setData(null); load(); }} className="underline ml-1">Retry</button>
            </div>
          )}
