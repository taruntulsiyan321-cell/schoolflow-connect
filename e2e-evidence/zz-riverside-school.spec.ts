...
test.describe('Riverside Public School · a school at work', () => {
  ...
  test('a class test: set for 8-A Mathematics, sat by the student, its mark read by the student and their parent', async ({ browser }) => {
    ...
    try {
      ...
      await t.getByText(/write questions manually/i).click()
      ...
      await t.getByPlaceholder('Correct option text *').fill('5')
      ...
      await expect(t.getByText(/test published successfully/i), 'the test is published for 8-A').toBeVisible({ timeout: 45000 })
