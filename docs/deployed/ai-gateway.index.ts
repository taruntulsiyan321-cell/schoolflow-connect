...
async function resolveActor(
  ...
  // WAS: a has_role() loop through this SERVICE-ROLE client, which answered
  // false for every role and every user, so this function returned null for
  // everyone and the gateway replied 403 actor_unresolved to all six student
  // surfaces. ai_request_decisions recorded nothing from 2026-08-25 onward.
  //
  // WHY IT ALWAYS FAILED (KNOWN_ISSUES 1): has_role/2 branches on auth.uid().
  // A service-role client carries no user JWT, so auth.uid() is NULL, the
  // cross-account branch is taken, and that branch compares
  // m.school_id = get_my_school_id() -- also NULL without a session. NULL = NULL
  // is never true, so every role answered false.
  //
  // Read memberships directly instead: service_role bypasses RLS, so it needs
  // no session, and it asks the question has_role was being used to ask. The
  // order mirrors ROLE_PRIORITY in src/auth/session.ts minus super_admin,
  // which memberships forbids by CHECK (role <> super_admin) anyway.
  ...
  // Transitional fallback, matching session.ts: an account predating the
  // memberships migration has no row there and must not be locked out.
  if (!role) {
