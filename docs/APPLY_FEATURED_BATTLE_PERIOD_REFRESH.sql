-- Featured battles: retire stale daily/weekly/ncert instances so each period
-- refreshes cleanly, and backfill questions on existing shells (classmates).

-- 1) One-shot: close prior-period featured rows still marked live/scheduled
...
-- 2) Allow same-class students to fill questions on featured shells
...
-- 3) Ensure: retire stale for this class, then create/return current period
...
BEGIN
  ...
  -- Retire prior-period featured for this class so unique indexes + list stay current
  IF _cid IS NOT NULL THEN
    UPDATE public.battles
      SET status = 'finished'
      WHERE class_id = _cid
        AND status IN ('live', 'scheduled')
        AND source = 'featured_daily'
        AND starts_at::date < CURRENT_DATE;
    UPDATE public.battles
      SET status = 'finished'
      WHERE class_id = _cid
        AND status IN ('live', 'scheduled')
        AND source = 'featured_ncert'
        AND starts_at::date < CURRENT_DATE;
    UPDATE public.battles
      SET status = 'finished'
      WHERE class_id = _cid
        AND status IN ('live', 'scheduled')
        AND source = 'featured_weekly'
        AND date_trunc('week', starts_at) < date_trunc('week', now());
  END IF;
  ...
  ELSIF _kind = 'teacher' THEN
    ...
    END IF;
    _bid := _existing;
    SELECT count(*) INTO _qcount FROM public.battle_questions WHERE battle_id = _bid;
    IF coalesce(_qcount, 0) = 0 THEN
