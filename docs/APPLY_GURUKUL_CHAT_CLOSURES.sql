...
-- DROP-safe: body now writes receipts + unread_count; avoid 42P13 on recreate.
DROP FUNCTION IF EXISTS public.rpc_mark_conversation_read(uuid);

CREATE FUNCTION public.rpc_mark_conversation_read(_conversation_id uuid)
RETURNS integer
LANGUAGE plpgsql
SECURITY DEFINER
...
CREATE FUNCTION public.rpc_list_conversations(
...
CREATE FUNCTION public.rpc_open_conversation(
...
CREATE FUNCTION public.rpc_search_chat(
  _query text,
...
-- Drop legacy zero-arg overload from features migration + product signature.
...
CREATE FUNCTION public.rpc_create_class_group(
  _class_id uuid,
...
CREATE FUNCTION public.rpc_create_teacher_group(_title text DEFAULT NULL)
RETURNS public.chat_conversations
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
