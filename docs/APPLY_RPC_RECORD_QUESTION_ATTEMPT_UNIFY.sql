-- Manual apply helper (Supabase SQL editor) — URGENT PostgREST overload fix.
-- Symptom: "Could not choose the best candidate function between THREE overloads
-- of public.rpc_record_question_attempt" when Practice records an attempt.
--
-- Root cause: PostgREST cannot disambiguate named-arg RPC calls when multiple
-- overloads share a common optional-arg prefix (10 / 12 / 13 args).
--
-- Fix: DROP every overload; keep ONE canonical signature with defaults.
-- Idempotent: safe to re-run.

-- ── 1) Drop ALL overloads by exact historical signatures ─────────────────────
...
-- Catch-all: any remaining overload with this name (different arg types/order)
DO $$
DECLARE
  r record;
BEGIN
...
-- Sanity: exactly one overload must remain
DO $$
DECLARE
  _n int;
BEGIN
  ...
  IF _n <> 1 THEN
    RAISE EXCEPTION 'rpc_record_question_attempt overload count = % (expected 1)', _n;
  END IF;
END $$;
